...
describe("TreeView", function() {
  ...
  let [treeView, path1, path2, root1, root2, sampleJs, sampleTxt, workspaceElement] = Array.from([]);
  ...
  afterEach(function() {
    ...
    if (treeViewOpenPromise = __guard__(atom.packages.getActivePackage('tree-view'), x => x.mainModule.treeViewOpenPromise)) {
  ...
  describe("when tree-view:reveal-active-file is triggered", function() {
    ...
    return describe('if there are more items than can be visible in the viewport', function() {
      let [rootDirPath] = Array.from([]);

      beforeEach(function() {
        rootDirPath = fs.absolute(temp.mkdirSync('tree-view-root1'));

        for (let i = 1; i <= 20; i++) {
          var filepath = path.join(rootDirPath, `file-${i}.txt`);
  ...
  describe("copy path commands", function() {
    let [pathToSelect, relativizedPath] = Array.from([]);
  ...
  describe("keyboard navigation", function() {
    ...
    describe("core:move-up", function() {
      describe("when there is an expanded directory before the currently selected entry", function() {
        let [directories, lastDir, fileAfterDir] = Array.from([]);
        beforeEach(function() {
    ...
    describe("tree-view:collapse-all", function() {
      const expandAll = () => (() => {
        const result = [];
        for (var root of Array.from(treeView.roots)) {
          root.expand(true);
          var children = root1.querySelectorAll('.directory');
      ...
      return _.each(splitOptions, function(options, direction) {
        ...
        return describe(command, function() {
          describe("when a file is selected", function() {
            ...
            it(`creates a new split pane ${direction}`, () => expect(previousPane.split).toHaveBeenCalledWith(...Array.from(options || [])));
  ...
  describe("file modification", function() {
    let [dirView, dirView2, dirView3, fileView, fileView2, fileView3, fileView4] = Array.from([]);
    let [rootDirPath, rootDirPath2, dirPath, dirPath2, dirPath3, filePath, filePath2, filePath3, filePath4] = Array.from([]);
    ...
    beforeEach(function() {
      ...
      root1 = treeView.roots[0];
      root2 = treeView.roots[1];
      [dirView, dirView2] = Array.from(root1.querySelectorAll('.directory'));
      dirView.expand();
      dirView2.expand();
      dirView3 = root2.querySelector('.directory');
      dirView3.expand();
      [fileView, fileView2, fileView3] = Array.from(root1.querySelectorAll('.file'));
    ...
    describe("tree-view:add-file", function() {
      let [addPanel, addDialog, callback] = Array.from([]);
      ...
      beforeEach(function() {
        ...
        return runs(function() {
          ...
          [addPanel] = Array.from(atom.workspace.getModalPanels());
    ...
    describe("tree-view:move", function() {
      describe("when a file is selected", function() {
        let [moveDialog, callback] = Array.from([]);
      ...
      describe("when a file is selected that's name starts with a '.'", function() {
        let [dotFilePath, dotFileView, moveDialog] = Array.from([]);
    ...
    describe("tree-view:duplicate", function() {
      ...
      describe("when a file is selected that's name starts with a '.'", function() {
        let [dotFilePath, dotFileView, copyDialog] = Array.from([]);
  ...
  describe("Git status decorations", function() {
    let [projectPath, modifiedFile, originalFileContent] = Array.from([]);
  ...
  describe("selecting items", function() {
    ...
    [dirView, fileView1, fileView2, fileView3, fileView4, fileView5, treeView, rootDirPath, dirPath, filePath1, filePath2, filePath3, filePath4, filePath5] = Array.from([]);
    ...
    beforeEach(function() {
      ...
      dirView.expand();
      return [fileView1, fileView2, fileView3, fileView4, fileView5] = Array.from(ref = dirView.querySelectorAll('.file')), ref;
    });
  ...
  describe("the sortFoldersBeforeFiles config option", function() {
    let [dirView, fileView, dirView2, fileView2, fileView3, rootDirPath, dirPath, filePath, dirPath2, filePath2, filePath3] = Array.from([]);
  ...
  describe("Dragging and dropping files", function() {
    let [rootDirPath, alphaDirPath, alphaFilePath, zetaFilePath, betaFilePath, etaDirPath, gammaDirPath,
     deltaFilePath, epsilonFilePath, thetaDirPath, thetaFilePath] = Array.from([]);
    ...
    describe("when dragging a FileView onto a DirectoryView's header", () => it("should add the selected class to the DirectoryView", function() {
      ...
      const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents([deltaFile], alphaDir.querySelector('.header'), null, treeView));
    ...
    describe("when dragging a FileView onto a FileView", () => it("should add the selected class to the parent DirectoryView", function() {
      ...
      const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents([deltaFile], betaFile, null, treeView));
      treeView.onDragStart(dragStartEvent);
      expect(deltaFile).toHaveClass('selected');
      treeView.onDragEnter(dragEnterEvent);
      expect(alphaDir).toHaveClass('selected');
    ...
    describe("when dropping a FileView onto a DirectoryView's header", function() {
      it("should move the file to the hovered directory", function() {
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] =
            Array.from(eventHelpers.buildInternalDragEvents([deltaFile], alphaDir.querySelector('.header'), alphaDir, treeView));
      ...
      describe('when the ctrl/cmd modifier key is pressed', () => it("should copy the file to the hovered directory", function() {
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] =
            Array.from(eventHelpers.buildInternalDragEvents([deltaFile], alphaDir.querySelector('.header'), alphaDir, treeView, true));
    ...
    describe("when dropping a FileView onto a FileView", function() {
      it("should move the file to the parent directory", function() {
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] =
            Array.from(eventHelpers.buildInternalDragEvents([deltaFile], betaFile, alphaDir, treeView));
    ...
    describe("when dropping multiple FileViews onto a DirectoryView's header", () => it("should move the files to the hovered directory", function() {
      ...
      const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents(gammaFiles, alphaDir.querySelector('.header'), alphaDir, treeView));
    ...
    describe("when dropping a DirectoryView and FileViews onto a DirectoryView's header", () => it("should move the files and directory to the hovered directory", function() {
      ...
      const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents(dragged, thetaDir.querySelector('.header'), thetaDir, treeView));
    ...
    describe("when dropping a DirectoryView onto a DirectoryView's header", function() {
      ...
      it("should move the directory to the hovered directory", function() {
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents([thetaDir], alphaDir.querySelector('.header'), alphaDir, treeView));
        treeView.onDragStart(dragStartEvent);
        treeView.onDrop(dropEvent);
        expect(alphaDir.children.length).toBe(2);
      ...
      it("shows a warning notification and does not move the directory if it would result in recursive copying", function() {
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents([alphaDir], etaDir.querySelector('.header'), etaDir, treeView));
        treeView.onDragStart(dragStartEvent);
        treeView.onDrop(dropEvent);
      ...
      it("shows a warning notification and does not move the directory if it would result in recursive copying (symlink)", function() {
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents([alphaDir], symlinkDir.querySelector('.header'), symlinkDir, treeView));
        treeView.onDragStart(dragStartEvent);
        treeView.onDrop(dropEvent);
      ...
      it("moves successfully when dragging a directory onto a sibling directory that starts with the same letter", function() {
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents([alphaDir], alpha2Dir.querySelector('.header'), alpha2Dir, treeView));
        treeView.onDragStart(dragStartEvent);
        treeView.onDrop(dropEvent);
      ...
      return it("moves successfully when dragging a symlink into its target directory", function() {
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents([symlinkDir], alphaDir.querySelector('.header'), alphaDir, treeView));
        treeView.onDragStart(dragStartEvent);
        treeView.onDrop(dropEvent);
        expect(alphaDir.children.length).toBe(2);
        alphaDir.reload();
    ...
    describe("when dropping a DirectoryView and FileViews onto the same DirectoryView's header", () => it("should not move the files and directory", function() {
      ...
      const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents(dragged, alphaDir.querySelector('.header'), alphaDir, treeView));
    ...
    describe("when dropping a DirectoryView and FileViews in the same parent DirectoryView", () => describe("when the ctrl/cmd modifier key is pressed", () => it("should copy the files and directory", function() {
      ...
      const [dragStartEvent, dragEnterEvent, dropEvent] =
          Array.from(eventHelpers.buildInternalDragEvents(dragged, alphaDir.querySelector('.header'), alphaDir, treeView, true));
    ...
    describe("when dragging entries that already exist", function() {
      ...
      describe("when dragging a single file", function() {
        let [dragStartEvent, dragEnterEvent, dropEvent] = Array.from([]);
        ...
        beforeEach(function() {
          ...
          return [dragStartEvent, dragEnterEvent, dropEvent] =
            Array.from(ref = eventHelpers.buildInternalDragEvents([deltaFile], alphaDir.querySelector('.header'), alphaDir, treeView)), ref;
        });
      ...
      describe("when dragging multiple files", function() {
        let [aAlphaFilePath, bAlphaFilePath, cAlphaFilePath,
         aGammaFilePath, bGammaFilePath, cGammaFilePath] = Array.from([]);
        let dropEvent = null;
      ...
      return describe("when dragging directories", function() {
        let [oldAFilePath, oldBFilePath, oldCFilePath, oldNestedDirPath, oldNestedFilePath,
         ...
         newCFilePath, newNestedDirPath, newNestedFilePath, onlyNewDirPath, onlyNewFilePath] = Array.from([]);
        let [dragStartEvent, dragEnterEvent, dropEvent] = Array.from([]);
        ...
        beforeEach(function() {
          ...
          return [dragStartEvent, dragEnterEvent, dropEvent] =
            Array.from(ref = eventHelpers.buildInternalDragEvents([newAlphaDir], treeView.roots[0].querySelector('.header'), treeView.roots[0], treeView)), ref;
        });
  ...
  describe("Dragging and dropping root folders", function() {
    let [alphaDirPath, gammaDirPath, thetaDirPath, etaDirPath] = Array.from([]);
    beforeEach(function() {
      const rootDirPath = fs.absolute(temp.mkdirSync('tree-view'));
    ...
    afterEach(function() {
      let ref;
      return [alphaDirPath, gammaDirPath, thetaDirPath, etaDirPath] = Array.from(ref = []), ref;});
    ...
    describe("when dragging a project root's header onto a different project root", function() {
      describe("when dragging on the top part of the root", () => it("should add the placeholder above the directory", function() {
        ...
        const [dragStartEvent, dragOverEvents, dragEndEvent] =
          Array.from(eventHelpers.buildPositionalDragEvents(gammaDir.querySelector('.project-root-header'), alphaDir, '.tree-view'));
      ...
      return describe("when below all entries", () => it("should add the placeholder below the last directory", function() {
        ...
        const [dragStartEvent, dragOverEvents, dragEndEvent] =
          Array.from(eventHelpers.buildPositionalDragEvents(alphaDir.querySelector('.project-root-header'), treeView.list));
    ...
    describe("when dropping a project root's header onto a different project root", function() {
      ...
      return describe("when dropping on the bottom part of the header", () => it("should add the placeholder below the directory", function() {
        ...
        const [dragStartEvent, dragDropEvents] =
          Array.from(eventHelpers.buildPositionalDragEvents(thetaDir.querySelector('.project-root-header'), alphaDir, '.tree-view'));
    ...
    describe("when a root folder is dragged out of application", () => it("should carry the folder's information", function() {
      const gammaDir = treeView.roots[1];
      const [dragStartEvent] = Array.from(eventHelpers.buildPositionalDragEvents(gammaDir.querySelector('.project-root-header')));
      treeView.rootDragAndDrop.onDragStart(dragStartEvent);
    ...
    describe("when a root folder is dropped from another Atom window", () => it("adds the root folder to the window", function() {
      let dragDropEvents;
      const alphaDir = treeView.roots[0];
      [_, dragDropEvents] = Array.from(eventHelpers.buildPositionalDragEvents(null, alphaDir.querySelector('.project-root-header'), '.tree-view'));
    ...
    describe("when a root folder is dropped to another Atom window", () => it("removes the root folder from the first window", function() {
      const gammaDir = treeView.roots[1];
      const [dragStartEvent, dropEvent] = Array.from(eventHelpers.buildPositionalDragEvents(gammaDir.querySelector('.project-root-header')));
      treeView.rootDragAndDrop.onDragStart(dragStartEvent);
...
describe("Service provider", function() {
  let [treeView, treeViewService] = Array.from([]);
  beforeEach(function() {
    waitForPackageActivation();
...
describe('Icon class handling', () => it('allows multiple classes to be passed', function() {
  ...
  const providerDisposable = atom.packages.serviceHub.provide('atom.file-icons', '1.0.0', {
    iconClassForPath(path, context) {
      ...
      const [name, id] = Array.from(path.match(/file-(\d+)\.txt$/));
      switch (id) {
