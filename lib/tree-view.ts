...
(TreeView = class TreeView {
  constructor(state) {
    ...
    this.disposables.add(this.onEntryMoved(({initialPath, newPath}) => {
      return (() => {
        const result = [];
        for (var editor of Array.from(atom.workspace.getTextEditors())) {
          var filePath = editor.getPath();
  ...
  showSelectedEntryInFileManager() {
    ...
    return atom.applicationDelegate.showItemInFolder(filePath);
  }
    ...
    return atom.confirm({
      ...
      if (response === 0) { // Move to Trash
        ...
        var trashNext = index => {
          ...
          return atom.applicationDelegate.moveItemToTrash(selectedPath).then(ok => {
            let repo;
            if (ok) {
